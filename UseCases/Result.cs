...
public readonly struct Result<TResponseValue>
{
    ...
    /// <param name="error"> Ошибка. </param>
    /// <returns> Результат выполнения команды. </returns>
    public static Result<TResponseValue> Unauthorized(string error) => new(new List<string>() { error },
    ...
    public static Result<TResponseValue> Unauthorized(IReadOnlyCollection<string> errors)
        => new(errors, ResultStatus.Unauthorized);
    /// <summary>
    ...
    /// <summary>
    /// Успешное выполнение, возрат результата не требуется
    /// </summary>
    /// <returns>Результат выполнения команды</returns>
